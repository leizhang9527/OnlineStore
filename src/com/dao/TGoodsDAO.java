...
public class TGoodsDAO extends HibernateDaoSupport
{
	...
	{
		log.debug("updating TGoods instance");
		try
		{
	...
	public TGoods findById(java.lang.Integer id)
	{
		...
		try
		{
			//TGoods instance = (TGoods) getHibernateTemplate().get("com.model.TGoods", id);
			List result = findByProperty("id",id);
			if(result==null || result.size() == 0)
			{
				return null;
			}
			else {
				return (TGoods)result.get(0);
			}
		} catch (RuntimeException re)
		{
